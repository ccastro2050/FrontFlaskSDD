...
- [ ] T008 Crear `services/auth_service.py` con la clase `AuthService` esqueleto (métodos: `login(email, contrasena)`, `cargar_roles_y_rutas(usuario_id)` con fallback según research.md §4, `cambiar_contrasena(id, actual, nueva)`, `recuperar_contrasena(email)`, `validar_contrasena_nueva(contrasena, email)` como función pura) y `descubrir_pks_fks(tabla)` placeholder
...
- [ ] T078 Revisar que ningún Blueprint loguee `session['token']` ni la contraseña plana; añadir test de regresión que inspecciona `caplog` en flujos de login/cambio de contraseña
...
- **Phase 2**: T009 (unit tests, archivo propio), T012–T020 (CSS + templates layout/components, archivos distintos) en paralelo tras T006–T008 y T010–T011. T021 depende de que T006–T011 existan.
